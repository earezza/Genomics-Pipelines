...
make_volcanoplot <- function(res, condition1, condition2, lfc_threshold, padj_threshold, subtitle='edgeR Results'){
...
for (c in colnames(combs)){
  ...
  edge <- DGEList(counts=mtx, group=mtx_info$Condition, remove.zeros=FALSE)
  ...
  if (opt$filter) {
    ...
    edge <- edge[keep,]
    cat("Percentiles after removing genes with sum(counts) <", opt$min_count, "\n")
    quantile(rowSums(edge$counts), probs = c(0, 0.1, 0.25, 0.5, 0.75, 0.9, 1))
  ...
  if (opt$filter){
    ...
    # cat("Genes and samples after filtering normalized genes:\n", dim(res), "\n")
    #write.table(res, file=paste(output_prefix, "count_mtx_normalized_filtered.csv", sep=""), sep=",", quote=F, col.names=NA)
    
    cat("Genes and samples before filtering normalized genes:\n", dim(normalized_count_mtx), "\n")
    cat("Removing genes with sum expression <", opt$min_basemean, "\n")
    norm_filtered_count_mtx <- subset(normalized_count_mtx, rowSums(normalized_count_mtx) > opt$min_basemean)
    cat("Genes and samples after filtering normalized genes:\n", dim(norm_filtered_count_mtx), "\n")
    
    # Write to file
    write.table(norm_filtered_count_mtx, file=paste(output_prefix, "count_mtx_normalized_filtered.csv", sep=""), sep=",", quote=F, col.names=NA)
    
    # Update edgeR object with filtered genes
    edge <- DGEList(counts = norm_filtered_count_mtx, group=mtx_info$Condition, remove.zeros=FALSE, norm.factors = edge$samples$norm.factors)
  }
  
  disp <- estimateDisp(edge)
  test<- exactTest(disp)
  res <- topTags(test, n=dim(test$table)[1], adjust.method="BH", sort.by="logFC", p.value=1)
  ...
  de.tags <- subset(res$table, res$table$FDR < 0.1)
  ...
  colnames(res$table) <- c('log2FoldChange', 'log2CPM', 'pvalue', 'padj')
  res$table[['FoldChange']] <- 2^abs(res$table[['log2FoldChange']])*(res$table[['log2FoldChange']]/abs(res$table[['log2FoldChange']]))
  res <- as.data.frame(res)
  
  write.table(res[order(res$log2FoldChange, decreasing=TRUE), ], file=paste(output_prefix, "edgeR_FullResult_", comparison, ".csv", sep=""), sep=",", quote=F, col.names=NA)
  ...
  plt <- make_volcanoplot(res, combs[[c]][1], combs[[c]][2], opt$lfc, 0.1, subtitle='edgeR Results')
  ...
  write.table(res_up_sorted, file=paste(out_dirs[combs[[c]][1]], "edgeR_Result_", combs[[c]][1], ".csv", sep=""), sep=",", quote=F, col.names=NA)
  ...
  write.table(res_down_sorted, file=paste(out_dirs[combs[[c]][2]], "edgeR_Result_", combs[[c]][2], ".csv", sep=""), sep=",", quote=F, col.names=NA)
  ...
  write.table(res_changed_sorted, file=paste(out_dirs[['DEGs']], "edgeR_Result_DEGs", ".csv", sep=""), sep=",", quote=F, col.names=NA)
